...
int main(int argc, char** argv)
{
    ...
    app.setSceneSetup([](SimLabApp& appInstance, WGPUDevice device, WGPUQueue queue) {
        ...
        auto incubator = makeIncubator<KinematicPhysicsEngine>()
            .withEnvironment([device, queue](SimScene& scene) {
                scene = SceneBuilder(device, queue)
                    // Concrete Ground Grid
                    .addGroundGrid(60.0f, 60.0f, 60)
                    // Target Goal Object (Anodized Red Metal Cube)
                    .addCube(
                        "target_goal",
                        Vec3{4.0f, 0.75f, -2.0f},
                        Vec3{1.2f, 1.2f, 1.2f},
                        Vec3{0.0f, 0.0f, 0.0f},
                        Material::Metallic({0.95f, 0.15f, 0.15f, 1.0f}, 0.20f),
                        true // isStatic
                    )
                    // Industrial Safety Barrier (High-Contrast Yellow)
                    .addCube(
                        "safety_barrier",
                        Vec3{-2.5f, 0.6f, 2.5f},
                        Vec3{3.5f, 1.2f, 0.2f},
        ...
        // Attach scene to 3D WebGPU visual renderer
